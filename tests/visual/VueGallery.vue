...
import { options, selectOptions, nav, paletteNav, treeNav, treeFoot, paletteItems, principal, locales, tiers, badgeTones, alertTones, menuItems, columns, rows, groups, article, createWorkspaceRows, buttonSizes, buttonVariants, progressValues, brands, workspaceMetrics, effectCaptions } from './parity-data';
const effectText = effectCaptions(!!document.documentElement.dataset.design);
const props = defineProps<{ scenario: string }>();
const accountIdentity = new URLSearchParams(location.search).get('accountIdentity') === 'true';
const tree = new URLSearchParams(location.search).get('tree') === 'true';
...
<template>
  ...
  <div v-else-if="scenario.startsWith('sidebar') && tree" class="shell-stage" data-component="ConsoleSidebar"><UI.ConsoleSidebar :model="treeNav" active-key="jobs" brand-name="Workspace" brand-sub="Console" search compact :open-key="null" :collapsed="scenario === 'sidebar-collapsed'" :foot-items="treeFoot"><template #logo><UI.LatereLogoMark/></template><template #foot><UI.AccountMenu :principal="principal" placement="bottom-start" subline="text"/></template></UI.ConsoleSidebar><div class="shell-content"><h2>Workspace overview</h2><p>Projects and activity appear beside the navigation.</p></div></div>
  <div v-else-if="scenario.startsWith('sidebar')" class="shell-stage" data-component="ConsoleSidebar"><UI.ConsoleSidebar :model="nav" active-key="jobs" brand-name="Workspace" brand-sub="Console" search :collapsed="scenario === 'sidebar-collapsed'"><template #logo><UI.LatereLogoMark/></template><template v-if="accountIdentity" #foot><UI.AccountMenu :principal="principal" placement="bottom-start"/></template></UI.ConsoleSidebar><div class="shell-content"><h2>Workspace overview</h2><p>Projects and activity appear beside the navigation.</p></div></div>
  <div v-else-if="scenario === 'palette'" data-component="ConsolePalette"><UI.GlassButton @click="open = true">Open palette</UI.GlassButton><UI.ConsolePalette :open="open" :model="tree ? treeNav : paletteNav" :items="tree ? paletteItems : undefined" @close="open = false"/></div>
  ...
  <div v-else-if="scenario === 'preferences'" class="stack"><div class="sample" data-component="AccountPrefs"><UI.AccountPrefs :theme="theme" :locale="locale" :locale-options="locales" @set-theme="theme = $event" @set-locale="locale = $event"/></div><section class="sample row preference-menus"><div data-component="ThemeMenu"><UI.ThemeMenu v-model:theme="theme" placement="bottom-start"/></div><div data-component="LocaleMenu"><UI.LocaleMenu v-model:locale="locale" :locales="locales" placement="bottom-start"/></div></section></div>
  ...
  <div v-else-if="scenario === 'logo'" class="logo-stage row" data-component="LatereLogoMark"><UI.LatereLogoMark/><UI.PlatformLogoMark data-component="PlatformLogoMark" width="48" height="48"/><span v-for="brand in brands" :key="brand" :class="`${brand}-brand`" style="font-size:28px">{{ brand }}</span></div>
