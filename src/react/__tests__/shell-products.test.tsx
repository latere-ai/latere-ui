...
describe('React ProductSwitcher', () => {
  it('renders registry marks, current non-link tile, labels, overrides and dismissal', () => {
    const view = render(<ProductSwitcher current="lectio" />);
    ...
    view.rerender(<ProductSwitcher current="lectio" products={LATERE_PRODUCTS.filter(p => ['lectio', 'identity'].includes(p.slug))} labels={{ switchProduct: 'Apps', products: 'Tools', current: 'Here' }} />);
  ...
  it('clamps when neither alignment fits and integrates into expanded sidebar only', () => {
    ...
    const view = render(<ConsoleSidebar model={{ groups: [] }} product="lectio" productLabels={{ switchProduct: 'Other apps' }} />);
    ...
    view.rerender(<ConsoleSidebar model={{ groups: [] }} product="lectio" collapsed />); expect(screen.queryByRole('button', { name: 'Other apps' })).toBeNull();
  });
});
