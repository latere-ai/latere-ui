...
export function AccountPrefs({ theme, locale, localeOptions, labels, onSetTheme, onSetLocale }: AccountPrefsProps) {
  ...
  return <div className="lu-ap">
    ...
    <div className="lu-ap-row" role="group" aria-label={t.language}>
      {localeOptions.map(opt => <button key={opt.code} type="button" className={`lu-ap-pill${locale === opt.code ? ' is-active' : ''}`} title={opt.name || opt.label} aria-pressed={locale === opt.code} onClick={() => onSetLocale?.(opt.code)}>{opt.label}</button>)}
    </div>
    <div className="lu-ap-label">{t.theme}</div>
    <div className="lu-ap-row" role="group" aria-label={t.theme}>
      {(['light', 'dark', 'auto'] as const).map(value => <button key={value} type="button" className={`lu-ap-pill${theme === value ? ' is-active' : ''}`} title={t[value]} aria-pressed={theme === value} onClick={() => onSetTheme?.(value)}>{t[value]}</button>)}
    </div>
  </div>;
}
