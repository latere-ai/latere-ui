...
- Console icons add `info`, `check-circle`, `alert-triangle`, `x-circle` and `more`, and `ConsoleIcon` is exported from both entrypoints, so a host draws them as the sidebar does. `identityParts` returns the role and the account the role subline states.
